"""
Handler for file_received events.

Processes file events and optionally triggers import jobs.
"""
...
class FileReceivedHandler(BaseHandler):
    """
    Handler for file_received events.

    Matches files against subscriber filters and triggers appropriate jobs.
    """

    def __init__(self):
        super().__init__("FileReceivedHandler")

    def can_handle(self, event_type: str) -> bool:
        """Only handle file_received events."""
        return event_type == 'file_received'

    def _get_matching_subscribers(self, event_data: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
        Get subscribers that match this file event.

        Args:
            event_data: Event data with file information
        ...
        # Get all active subscribers for file_received events
        query = """
            SELECT
                s.subscriber_id,
                s.subscriber_name,
            ...
            AND s.event_type = 'file_received'
        """
        subscribers = fetch_dict(query) or []

        # Filter by event_filter (e.g., file_pattern)
        filename = event_data.get('filename', '')
        matching = []

        for sub in subscribers:
            event_filter = sub.get('event_filter', {}) or {}

            # Check file_pattern filter
            file_pattern = event_filter.get('file_pattern', '*')
            if fnmatch.fnmatch(filename, file_pattern):
                matching.append(sub)
                logger.debug(f"Subscriber {sub['subscriber_name']} matches file {filename}")
            else:
                logger.debug(f"Subscriber {sub['subscriber_name']} does not match file {filename}")
    ...
    def handle(self, event_data: Dict[str, Any]) -> bool:
        ...
        """
        self.log_event(event_data, f"Processing file: {event_data.get('filename', 'unknown')}")

        subscribers = self._get_matching_subscribers(event_data)
