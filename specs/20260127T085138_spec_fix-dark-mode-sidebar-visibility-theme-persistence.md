# Plan: Fix Dark Mode Sidebar Visibility & Theme Persistence

## Overview

Fix two critical dark mode issues in the Tangerine admin interface:
1. **Sidebar Navigation Headers Not Visible**: Group headers (Home, Configuration, Operations, System) are hard to see in dark mode
2. **Theme Doesn't Persist**: Theme resets to light mode after browser refresh
...
**Current Behavior:**
- Navigation group headers ("Home", "Configuration", "Operations", "System") appear as dark gray in dark mode
- Headers blend into the sidebar background (#1E1E2E)
- User cannot easily distinguish section groups

**Root Cause:**
- **File**: `/opt/tangerine/admin/styles/custom.css`
- **Lines 1061-1064**: Wildcard selector overrides specific header rules
```css
[data-theme="dark"] [data-testid="stSidebar"] *,
.stApp[data-theme="dark"] [data-testid="stSidebar"] * {
    color: #E8E8E8 !important;  /* This catches EVERYTHING including headers */
}
```
- **Lines 1067-1078**: More specific rules for headers exist but are overridden
- The wildcard `*` selector has equal `!important` specificity but loads later, winning the cascade

**Expected Behavior:**
- Headers should be bright white (#FFFFFF) or Tangerine orange (#FFA05C)
- Clear visual distinction from regular navigation links
- Sufficient contrast against sidebar background
...
### Solution 1: Fix Dark Mode Sidebar Visibility

**Approach: Remove Wildcard, Enhance Specific Selectors**

The wildcard selector at lines 1061-1064 must be removed or made more specific to exclude navigation headers.

**Option A (Recommended): Remove Wildcard**
Delete lines 1061-1064 entirely and rely on specific selectors for each element type.

**Option B: Exclude Headers from Wildcard**
Make wildcard more specific to avoid catching headers:
```css
/* Target specific elements, not everything */
[data-theme="dark"] [data-testid="stSidebar"] p:not([data-nav-header]),
[data-theme="dark"] [data-testid="stSidebar"] span:not([data-nav-header]),
[data-theme="dark"] [data-testid="stSidebar"] a {
    color: #E8E8E8 !important;
}
```

**Enhanced Header Selectors (Lines 1067-1078):**
Add more selectors to ensure headers are caught:
```css
/* Use brighter color for maximum visibility */
[data-theme="dark"] [data-testid="stSidebar"] [data-testid="stSidebarNav"] ul li p,
[data-theme="dark"] [data-testid="stSidebar"] [data-testid="stSidebarNav"] > div > ul > li > div,
[data-theme="dark"] [data-testid="stSidebar"] [data-testid="stSidebarNav"] > div > div > div:first-child,
[data-theme="dark"] [data-testid="stSidebar"] nav[role="navigation"] > div > div > span,
[data-theme="dark"] [data-testid="stSidebar"] span[role="heading"] {
...
### Solution 2: Implement Theme Persistence
...
### Issue 1 Implementation: Fix Sidebar Visibility

**Step 1: Remove Wildcard Override**
- **File**: `/opt/tangerine/admin/styles/custom.css`
- **Action**: DELETE lines 1061-1064
```css
...
**Step 2: Enhance Header Selectors**
- **File**: `/opt/tangerine/admin/styles/custom.css`
- **Action**: REPLACE lines 1067-1078 with enhanced selectors
```css
/* Dark mode - sidebar navigation group headers with multiple fallback selectors */
[data-theme="dark"] [data-testid="stSidebar"] [data-testid="stSidebarNav"] ul li p,
[data-theme="dark"] [data-testid="stSidebar"] [data-testid="stSidebarNav"] > div > ul > li > div,
[data-theme="dark"] [data-testid="stSidebar"] [data-testid="stSidebarNav"] > div > div > div:first-child,
[data-theme="dark"] [data-testid="stSidebar"] nav[role="navigation"] > div > div > span,
[data-theme="dark"] [data-testid="stSidebar"] span[role="heading"],
.stApp[data-theme="dark"] [data-testid="stSidebar"] [data-testid="stSidebarNav"] ul li p {
    color: #FFFFFF !important;  /* Bright white for maximum visibility */
    font-weight: 700 !important;
    font-size: 0.95rem !important;
    letter-spacing: 0.5px !important;
...
**Step 3: Ensure Links Stay Distinct**
- **File**: `/opt/tangerine/admin/styles/custom.css`
- **Action**: Verify lines 1080-1097 remain unchanged (links should be dimmer)

### Issue 2 Implementation: Add Theme Persistence

**Step 1: Add JavaScript Theme Loader**
- **File**: `/opt/tangerine/admin/app.py`
- **Action**: INSERT after line 28
```python
# Initialize theme from localStorage if available
if 'theme' not in st.session_state:
    # Try to read saved theme preference
    st.markdown("""
        <script>
        const savedTheme = localStorage.getItem('tangerine_theme') || 'light';
        // Create hidden element to pass data to Streamlit
        const themeData = document.createElement('div');
        themeData.id = 'theme-loader';
        themeData.setAttribute('data-saved-theme', savedTheme);
        themeData.style.display = 'none';
        document.body.appendChild(themeData);
        </script>
    """, unsafe_allow_html=True)

    # Default to light (will be overridden if preference exists)
    st.session_state.theme = 'light'
```

**Step 2: Save Theme on Toggle**
- **File**: `/opt/tangerine/admin/app.py`
- **Action**: REPLACE lines 112-117
```python
...
### Issue 1: Sidebar Visibility Testing

**Manual Visual Test:**
1. Toggle dark mode ON
2. Inspect sidebar navigation headers:
   - "Home" should be bright white or orange
   - "Configuration" should be bright white or orange
   - "Operations" should be bright white or orange
   - "System" should be bright white or orange
3. Verify contrast is sufficient (readable without squinting)
4. Ensure navigation links (Dashboard, Imports, etc.) remain dimmer than headers

**Browser DevTools Test:**
```javascript
// Open DevTools Console, run:
const headers = document.querySelectorAll('[data-testid="stSidebar"] [data-testid="stSidebarNav"] ul li p');
headers.forEach(h => {
    const color = getComputedStyle(h).color;
    console.log(`Header: ${h.textContent}, Color: ${color}`);
});
// Expected: rgb(255, 255, 255) for white or rgb(255, 160, 92) for orange
```

**Cross-Browser Test:**
- Chrome: Verify headers visible
- Firefox: Verify headers visible
- Edge: Verify headers visible

### Issue 2: Theme Persistence Testing

**Manual Persistence Test:**
1. Open app (defaults to light mode)
2. Toggle dark mode ON
3. Verify UI changes to dark
4. Press F5 or Ctrl+R to refresh browser
5. **VERIFY**: App loads in dark mode (not light)
6. Toggle light mode ON
7. Refresh browser
8. **VERIFY**: App loads in light mode

**localStorage Verification:**
```javascript
// Open DevTools Console
// Check if theme is stored:
console.log(localStorage.getItem('tangerine_theme'));
// Expected: 'dark' or 'light'

// Manually test persistence:
localStorage.setItem('tangerine_theme', 'dark');
// Refresh page, verify dark mode loads
```

**Edge Cases:**
1. **Incognito Mode**: localStorage may be disabled → should default to light
2. **Multiple Tabs**: Change theme in Tab 1, refresh Tab 2 → should reflect change
3. **Clear localStorage**: Delete key, refresh → should default to light
...
✅ **Visual Verification:**
- [ ] Group headers clearly visible in dark mode
- [ ] Headers use bright white (#FFFFFF) or Tangerine orange (#FFA05C)
- [ ] Sufficient contrast against sidebar background (#1E1E2E)
- [ ] Navigation links remain distinct (dimmer gray)

✅ **Technical Verification:**
- [ ] Wildcard selector removed from CSS (lines 1061-1064 deleted)
- [ ] Enhanced selectors targeting headers (lines 1067-1078 updated)
...
- [ ] Fresh install → Defaults to light
- [ ] localStorage contains `'tangerine_theme': 'dark'` or `'light'`

✅ **Technical Verification:**
- [ ] JavaScript executes without errors
- [ ] Console shows `[Theme] Saved: dark/light` on toggle
- [ ] Theme loads within 500ms of page load
...
1. **`/opt/tangerine/admin/styles/custom.css`**
   - Lines 1061-1064: DELETE wildcard selector
   - Lines 1067-1078: REPLACE with enhanced selectors

### Issue 2 Files
1. **`/opt/tangerine/admin/app.py`**
   - After line 28: INSERT JavaScript theme loader
   - Lines 112-117: REPLACE toggle handler with persistence logic
...
### Issue 1 Risks
**Low Risk**: CSS-only changes, non-breaking
- **Risk**: Selectors may not match all Streamlit DOM variations
- **Mitigation**: Use multiple fallback selectors (`ul li p`, `span[role="heading"]`, etc.)

### Issue 2 Risks
**Medium Risk**: JavaScript injection may have timing issues
- **Risk**: JavaScript may execute before Streamlit is ready
- **Mitigation**: Use synchronous script injection, add console logging for debugging
- **Fallback**: Query parameter approach if localStorage doesn't work

**Low Risk**: localStorage may be disabled in some browsers
- **Risk**: Incognito mode, corporate policies
- **Mitigation**: Graceful degradation to session-only (current behavior)
...
3. **Test Issue 1**:
   - Toggle dark mode
   - Visually inspect sidebar headers (should be white/orange)
   - Take screenshot for comparison
4. **Test Issue 2**:
   - Toggle to dark mode
   - Refresh browser (F5)
   - Verify dark mode persists
   - Check DevTools Console for localStorage value
5. **Cross-Browser Test**:
   - Repeat steps in Chrome, Firefox, Edge
6. **Edge Case Test**:
...
- **Issue 1 Fix**: 15-30 minutes (CSS changes + testing)
- **Issue 2 Fix**: 30-60 minutes (JavaScript integration + testing)
- **Total**: 1-1.5 hours including thorough testing
