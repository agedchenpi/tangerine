"""
Fetch and import Yahoo Finance global market index OHLCV data.

Fetches the most recent daily price for 7 major global market indexes
(FTSE 100, DAX, CAC 40, Euro Stoxx 50, Nikkei 225, Hang Seng, ASX 200)
via the yfinance library.

Usage:
    python etl/jobs/run_yfinance_global_indexes.py
    python etl/jobs/run_yfinance_global_indexes.py --dry-run
"""
...
CONFIG_NAME = 'YFinance_Global_Indexes'

logger = get_logger('run_yfinance_global_indexes')


def transform(raw_data: list) -> list:
    """Validate and add audit columns to global index price records."""
    transformed = []
    ...
    logger.info(f"Transformed {len(transformed)} global index price records")
    return transformed


def main():
    parser = argparse.ArgumentParser(description='Fetch and import Yahoo Finance global market index prices')
    parser.add_argument('--dry-run', action='store_true', help='Fetch and save but do not load to database')
    args = parser.parse_args()

    with JobRunLogger('run_yfinance_global_indexes', CONFIG_NAME, args.dry_run) as job_log:
        step_id = job_log.begin_step('data_collection', 'Data Collection')
        try:
            client = YFinanceClient()
            raw_data = client.get_global_index_prices()
            if not raw_data:
