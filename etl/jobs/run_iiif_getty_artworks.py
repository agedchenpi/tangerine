...
For each manifest UUID, fetches the IIIF Presentation API manifest from
media.getty.edu, extracts structured artwork metadata, downloads the
full-resolution image, and imports the records into feeds.tiiif_artwork
via generic_import.
...
def collect_records(dry_run: bool) -> list:
    ...
    with IIIFClient() as client:
        for manifest_uuid in MANIFEST_IDS:
